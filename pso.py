...
class PSO:
    def __init__(self, 
        ...
        self.c4 = c4
        self.max_g = max_g
        self.max_g_no_improvement = max_g_no_improvement
        self.distancing = distancing
        self.fit_weight = fit_weight
        self.landscape = landscape

        self.p_best = self.swarm
        self.p_best_val = self.fitness(self.swarm, self.N, self.landscape)

        self.best_fit = np.sum(self.fitness(self.swarm, self.N, self.landscape))
        self.coverage = 0
    ...
    def update_best(self):
        fits = self.fitness(self.swarm, self.N, self.landscape)
        n = len(self.swarm)
        for i in range(n):
        ...
        if (np.sum(fits) * self.fit_weight - self.coverage * (1-self.fit_weight)) < (self.best_fit * self.fit_weight - self.coverage * (1-self.fit_weight)):
            ...
            if self.iter_no_improvement == self.max_g_no_improvement:
